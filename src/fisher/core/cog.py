...
class FisherCog(Cog, Singleton, metaclass=SingletonCogMeta):
    ...
    def __check_db(self):
        if not self.requires_db:
            raise RuntimeError(
                f"Cog [{type(self).__name__}] does not require a database but tried to access it. "
                "Please set requires_db to True to use database-related methods."
            )
    ...
    async def _register_group_corpus(self, group: app_commands.Group) -> None:
        ...
        if "description" in group.extras["locale"]:
            self.__corpus.add_group_description(group.description, group.extras["locale"]["description"])
    ...
    async def _register_command_corpus(self, command: app_commands.Command) -> None:
        ...
        if "name" in command.extras["locale"]:
            self.__corpus.add_command_name(command.name, command.extras["locale"]["name"])
        if "description" in command.extras["locale"]:
            self.__corpus.add_command_description(command.description, command.extras["locale"]["description"])
        await self._register_parameters_corpus(command.parameters, command.extras["locale"].get("parameters", {}))
    ...
    async def _register_parameters_corpus(
        ...
        for parameter in parameters:
            ...
            if "name" in locale_dict[parameter.name]:
                self.__corpus.add_parameter_name(parameter.name, locale_dict[parameter.name]["name"])
            ...
            await self._register_choices_corpus(parameter.choices, locale_dict[parameter.name].get("choices", {}))

    async def _register_choices_corpus(self, choices: list[app_commands.Choice], locale_dict: dict[str, dict]):
        for choice in choices:
            if choice.name not in locale_dict:
                continue
            if "name" in locale_dict[choice.name]:
                self.__corpus.add_choice_name(choice.name, locale_dict[choice.name]["name"])
